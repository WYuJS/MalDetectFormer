...
class FEDformer(nn.Module):
    ...
    """
    def __init__(self, enc_in, dec_in, c_out, output_size, seq_len=2, label_len=48,  pred_len=2, L=3, base='legendre',
                factor=1, d_model=512, n_heads=8, e_layers=3, d_layers=2,mode_select='random',
                dropout=0.2, embed='timeF', freq='h', activation='gelu',version='Fourier',modes=64, moving_avg=[24],
        ...
        if isinstance(kernel_size, list):
            self.decomp = series_decomp_multi(kernel_size).to(device)
        else:
            self.decomp = series_decomp(kernel_size).to(device)
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,is_multi,
        ...
        x_enc = Reshape(x_enc, self.seq_len, self.feature_size)
        x_dec = Reshape(x_dec, self.seq_len, self.feature_size)
        x_mark_enc = Reshape(x_mark_enc, self.seq_len, x_mark_enc.shape[1])
        x_mark_dec = Reshape(x_mark_dec, self.seq_len, x_mark_dec.shape[1])
        # decomp init
        ...
        # torch.save(attns, 'fedformer_attn_weights.pt')
