...
def determine_number_type(s):
    try:
        ...
        pass  # 如果尝试转换为int失败，继续尝试转换为float

    try:
        float(s)
        return "float"
    except ValueError:
        return "not a number"  # 如果两者都失败，返回 not a number
...
def getUNSWData(args, is_multi=False):
    # unsw文件是数值型的编号
    unsw_numeric_list = np.array([7, 8, 9, 15, 16, 17, 18, 23, 24, 27, 28, 31, 32, 33, 34, 35, 38, 40, 41, 42, 43, 44, 45, 46,
                         47]) - 1 + 2
    # 设定文件夹路径，例如 "./data/"
...
def getids2017(args, is_multi):
    ...
    if is_multi:
        return data.float().to(device), type_labels, time.float().to(device)
    else:
        return data.float().to(device), anomaly_labels, time.float().to(device)
